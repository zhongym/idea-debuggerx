...
import com.github.zhongym.debuggerx.java.EnhancedJavaLineBreakpointProperties
...
internal class EnhancedBreakpointPanel(
  ...
  private val evaluateButton = JBRadioButton(EnhancedDebuggerBundle["panel.returnType.evaluate"])
  ...
  override fun saveTo(breakpoint: XLineBreakpoint<JavaLineBreakpointProperties>) {
    ...
    if (properties is EnhancedJavaLineBreakpointProperties) {
      ...
      properties.setForceType(if (valueButton.isSelected) ForceType.VALUE else if (exceptionButton.isSelected) ForceType.EXCEPTION else ForceType.EVALUATE)
    }
  }
  ...
  override fun loadFrom(breakpoint: XLineBreakpoint<JavaLineBreakpointProperties>) {
    ...
    forceReturnCheckBox.addItemListener {
      ...
      evaluateButton.isEnabled = isSelected
    }

    ButtonGroup().also {
      it.add(valueButton)
      it.add(exceptionButton)
      it.add(evaluateButton)
    }
